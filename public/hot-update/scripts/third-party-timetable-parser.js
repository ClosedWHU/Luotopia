// Parses third-party timetables (ShiGuang / XingLian) into a normalized list.
function parse(rawJson) {
  ...
  // Build a lookup of section -> start/end time from timeSlots.
  var slots = {}, rawSlots = Array.isArray(data.timeSlots) ? data.timeSlots : [];
  for (var i = 0; i < rawSlots.length; i++) {
    var slot = rawSlots[i];
    // ShiGuang uses "number" for section; XingLian uses "section".
    var section = asInt(slot[shiguang ? 'number' : 'section']);
    if (section !== null && typeof slot.startTime === 'string' && typeof slot.endTime === 'string') {
      slots[section] = { start: slot.startTime, end: slot.endTime };
    }
  }
  var courses = [];
  for (var j = 0; j < rawCourses.length; j++) {
    ...
    // Prefer a custom time, then a direct time, then derive from the section.
...
// Converts a week value (array of numbers or raw) into a "1、2、3" style label.
