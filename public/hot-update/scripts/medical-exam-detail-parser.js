// Parses an examination report detail page: title, header fields, and
// labeled description sections (检查描述/检查所见/检查结果).
function parse(rawJson) {
  var html = text(JSON.parse(rawJson).html), title = first(html, /<p[^>]*class=["'][^"']*font6[^"']*f18[^"']*["'][^>]*>([\s\S]*?)<\/p>/i), fields = {}, sections = [];
  // Header block with department and publish time.
  ...
  // Description sections: label followed by content in the next <p>.
